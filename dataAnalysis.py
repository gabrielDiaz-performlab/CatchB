...
def ParseExpData(TextFileName):
    ...
    # "exp_data-2014-5-9-13-9 copy.txt"
    txtFile = open(TextFileName + '.txt', "r")
    ...
    for aline in txtFile:
        ...
        for i in range(len(Line)):
            ...
            if (Line[i] == 'frameTime'):
                ...
                eyePOR_XY = np.vstack((eyePOR_XY, np.array(
                    [float(Line[i + 1]), float(Line[i + 2])])))
            elif (Line[i] == 'eyeGazeDir_XYZ'):
                eyeGazeDir_XYZ = np.vstack((eyeGazeDir_XYZ, np.array(
                    [float(Line[i + 1]), float(Line[i + 2]), float(Line[i + 3])])))
            elif (Line[i] == 'eyeGazePoint_XYZ'):
                eyeGazePoint_XYZ = np.vstack((eyeGazePoint_XYZ, np.array(
                    [float(Line[i + 1]), float(Line[i + 2]), float(Line[i + 3])])))

            elif (Line[i] == 'rightPupilRadius'):
                rightPupilRadius = np.hstack((rightPupilRadius, Line[i + 1]))
            elif (Line[i] == 'rightEyeLensDistance'):
                rightEyeLensDistance = np.hstack(
                    (rightEyeLensDistance, Line[i + 1]))
            elif (Line[i] == 'rightPOR_XY'):
                rightPOR_XY = np.vstack((rightPOR_XY, np.array(
                    [float(Line[i + 1]), float(Line[i + 2])])))
            elif (Line[i] == 'rightGazeDir_XYZ'):
                rightGazeDir_XYZ = np.vstack((rightGazeDir_XYZ, np.array(
                    [float(Line[i + 1]), float(Line[i + 2]), float(Line[i + 3])])))
            elif (Line[i] == 'rightGazePoint_XYZ'):
                rightGazePoint_XYZ = np.vstack((rightGazePoint_XYZ, np.array(
                    [float(Line[i + 1]), float(Line[i + 2]), float(Line[i + 3])])))
            elif (Line[i] == 'rightPupilPos_XYZ'):
                rightPupilPos_XYZ = np.vstack((rightPupilPos_XYZ, np.array(
                    [float(Line[i + 1]), float(Line[i + 2]), float(Line[i + 3])])))

            elif (Line[i] == 'leftPupilRadius'):
                leftPupilRadius = np.hstack((leftPupilRadius, Line[i + 1]))
            elif (Line[i] == 'leftEyeLensDistance'):
                leftEyeLensDistance = np.hstack(
                    (leftEyeLensDistance, Line[i + 1]))
            elif (Line[i] == 'leftPOR_XY'):
                leftPOR_XY = np.vstack((leftPOR_XY, np.array(
                    [float(Line[i + 1]), float(Line[i + 2])])))
            elif (Line[i] == 'leftGazeDir_XYZ'):
                leftGazeDir_XYZ = np.vstack((leftGazeDir_XYZ, np.array(
                    [float(Line[i + 1]), float(Line[i + 2]), float(Line[i + 3])])))
            elif (Line[i] == 'leftGazePoint_XYZ'):
                leftGazePoint_XYZ = np.vstack((leftGazePoint_XYZ, np.array(
                    [float(Line[i + 1]), float(Line[i + 2]), float(Line[i + 3])])))
            elif (Line[i] == 'leftPupilPos_XYZ'):
                leftPupilPos_XYZ = np.vstack((leftPupilPos_XYZ, np.array(
                    [float(Line[i + 1]), float(Line[i + 2]), float(Line[i + 3])])))

            elif (Line[i] == 'viewPos_XYZ'):
                view_XYZ_Matrix = np.vstack(
                    (view_XYZ_Matrix, np.array([Line[i + 1], Line[i + 2], Line[i + 3]])))
            elif (Line[i] == 'viewQUAT_WXYZ'):
                view_Quat_Matrix = np.vstack((view_Quat_Matrix, np.array(
                    [Line[i + 1], Line[i + 2], Line[i + 3], Line[i + 4]])))
            elif (Line[i] == 'paddlePos_XYZ'):
                Paddle_XYZ_Matrix = np.vstack(
                    (Paddle_XYZ_Matrix, np.array([Line[i + 1], Line[i + 2], Line[i + 3]])))
            elif (Line[i] == 'paddleQUAT_WXYZ'):
                Paddle_Quat_Matrix = np.vstack((Paddle_Quat_Matrix, np.array(
                    [Line[i + 1], Line[i + 2], Line[i + 3], Line[i + 4]])))
            elif (Line[i] == 'ballPos_XYZ'):
                Ball_XYZ_Matrix = np.vstack(
                    (Ball_XYZ_Matrix, np.array([Line[i + 1], Line[i + 2], Line[i + 3]])))
            elif (Line[i] == 'ballVel_XYZ'):
                Ball_Vel_XYZ_Matrix = np.vstack(
                    (Ball_Vel_XYZ_Matrix, np.array([Line[i + 1], Line[i + 2], Line[i + 3]])))
            elif (Line[i] == 'ballPix_XYDist'):
                BallPix_XYDist = np.vstack(
                    (BallPix_XYDist, np.array([Line[i + 1], Line[i + 2], Line[i + 3]])))
...
def update_line1(num, sessionData, line):
    ...
    line.set_marker('p')
    line.set_data((sessionData['rawData_tr']['rightPupilPos_XYZ'][num, 0],
                   sessionData['rawData_tr']['rightPupilPos_XYZ'][num, 1]))
    if num > 11445:
        print 'F=', num
    return line,
...
def update_line2(num, sessionData, line):
    ...
    line.set_marker('*')
    line.set_data((sessionData['rawData_tr']['leftPupilPos_XYZ'][num, 0],
                   sessionData['rawData_tr']['leftPupilPos_XYZ'][num, 1]))
    if num > 11445:
        print 'F=', num
    return line,
...
if __name__ == "__main__":
    ...
    sessionData = {'rawData_tr': None, 'processedData_tr': None,
                   'expInfo': None, 'dependentMeasures_tr': None}
    sessionData['rawData_tr'] = sio.loadmat(
        'Exp_RawMat_' + TextFileName + '.mat')

    # , sessionData['rawData_tr']['rightPupilPos_XYZ'][0][1], sessionData['rawData_tr']['rightPupilPos_XYZ'][0][2]
    print 'Data =  ', sessionData['rawData_tr']['rightPupilPos_XYZ'][1000:1020, 2]
    ...
    plt.grid(True)
    line_ani = animation.FuncAnimation(fig1, update_line1, frames=11448, fargs=(
        sessionData, l1), interval=14, blit=True)
    plt.show()

...
viz.MainView.setPosition([0,.01,-2])
viz.MainView.lookAt([0,.1,0])        
viz.vsync(1)
room = room()

radius = 0.5
ballInitialPos = [0,radius,0]
ballInitialVel = [0,0,0]


ball = visObj(room,'sphere',radius,ballInitialPos,[0,1,1])
ball.enablePhysNode()
ball.setVelocity(ballInitialVel)
ball.physNode.enableMovement()
ball.linkToPhysNode()

#ball.projectShadows(room.floor.node3D) # Costly, in terms of computation

def setBallRadius(newRadius):
    
    ballInitialPos = [0,newRadius*2.0,0]
    
    # Initial diameter of 1 meterr
    mat = ball.node3D.getMatrix()
    
    for idx in range(0,11,5):
        mat[idx] = newRadius*2.0
    
    ball.node3D.setMatrix(mat)
    ball.physNode.geom.setRadius(newRadius)
    
    ball.physNode.geomMass.setSphereTotal(1, radius)
    ball.physNode.body.setMass(ball.physNode.geomMass)
